#!/usr/bin/env python3
"""Check + fix node config + test Wings connection."""
...
$n = Node::first();
echo "BEFORE: scheme={$n->scheme} fqdn={$n->fqdn} daemonListen={$n->daemonListen}\\n";
echo "getConnectionAddress: " . $n->getConnectionAddress() . "\\n";

// Set node to localhost:8080 for Panel->Wings communication
$n->scheme = 'http';
$n->fqdn = '127.0.0.1';
$n->daemonListen = 8080;
$n->daemonSFTP = 2022;
...
$fresh = Node::first();
echo "\\nAFTER: scheme={$fresh->scheme} fqdn={$fresh->fqdn} daemonListen={$fresh->daemonListen}\\n";
echo "getConnectionAddress: " . $fresh->getConnectionAddress() . "\\n";
"""
print("=== Fix node config ===")
print(php_run(fix_php, timeout=30))
...
# Test Panel->Wings API call
print("\n=== Test Panel->Wings API ===")
print(run("curl -s --max-time 10 -H 'Authorization: Bearer FZ7Nz3jEY8Y1KP4NAfVrP7CV7nkZpGYzWGAZYr4x2bcVoUYfLqn82TgMd7mUp2f6' http://127.0.0.1:8080/api/servers 2>&1 | head -c 200", timeout=15))
